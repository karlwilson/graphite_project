...
zp = 10;      // cells in flow direction (chamber)
nhp = 28;    // points across each chamber outer half-band (graded toward jet)
exp = 1.07;  // grading progression toward jet axis

// Jet nozzle (charging zone)
wc = 0.0072; // nozzle width
dc = 0.075;  // nozzle length
zc = 21;     // cells in flow direction (nozzle / back region)
nhc = 7;     // points across nozzle half-width
tc = 0.001;  // nozzle wall thickness (solid, un-meshed). 2D: flat wall, no sqrt(2)
ntc = 1;     // points across each nozzle wall band
