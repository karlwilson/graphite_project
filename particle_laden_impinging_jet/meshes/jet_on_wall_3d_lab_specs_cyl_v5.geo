...
// Cylindrical jet in a cylindrical chamber. v5 is v4 refined uniformly by a
// factor of 3 for unresolved CFD-DEM with 100 um particles. Geometry (all
// radii and depths), the balanced-ball bore, the three 2:1 transition rings
// and arT are unchanged; only the discretisation counts move.
//
// Why 3 and not more: unresolved CFD-DEM needs every cell to stay comfortably
// larger than a particle, and the binding edge here is the bore pinwheel's
// outer-ring radial, 0.708 mm in v4. Dividing by 3 gives 0.236 mm = 2.36 dp,
// which clears the 2 dp floor; a factor of 4 would give 0.177 mm and put
// cells below the particle diameter.
//
// The refinement is exactly uniform, so the aspect-ratio structure v4 was
// built for carries over unchanged: dz, the circumferential chord and every
// radial spacing all shrink by 3 together, which leaves the transition radii
// tR = arT*dz/CH at (very nearly) their v4 values and the AR profile
// identical -- bore AR ~1.1, max AR ~11.
//
// v4 background, still true here: v3 shared one axial count across the whole
// cross-section, so AR(r)/AR(bore) = r/r_bore = 114 was fixed by geometry and
// an isotropic bore forced AR 87 at Rc2. Letting dz vary with radius through
// three 2:1 transition rings breaks that. Each ring is a conforming 3-quad
// template (a hanging node made conforming - deal.II cannot read a level-0
// hanging node, see GridIn's documentation, and would silently treat it as an
// internal crack).
//
...
// "Coherence Mesh" both leave those nodes unmerged. Build with build_v5.py,
...
// Entity-by-entity duplicate checking is O(n^2) here (about 9000 revolved
// volumes, 3x v4's count); switching it off takes the geometry stage from
// minutes to seconds.
...
nb   = 7;      // points per 45deg arc  (circumferential; also inner-quad edges)
nbr  = 7;      // points across a bore cap (radial)
ntc  = 4;      // points across the nozzle wall thickness (gmsh clamps to >= 2,
               // which is why v4's ntc = 1 still gave one cell)

// Axial counts. Both MUST be divisible by 2^(number of transition rings) = 8.
// dz is then ~0.29 mm, just under the central pinwheel's in-plane edge range
// (0.283-0.316 mm), so the bore cells stay isotropic as in v4.
zp   = 168;    // z cells in the projection zone at factor 1 (forward, +dp)
zc   = 264;    // z cells in the charging zone   at factor 1 (backward, -dc)
...
// circumferential chord per cell, divided by r. There are 8*(nb-1) cells
// around the full circle (8 sectors of nb-1); v4 hard-coded the nb = 3 value.
CH  = 2 * Sin(Pi/(8*(nb-1)));
...
// The weld (removeDuplicateNodes) happens in build_v5.py, after meshing:
